...
- [ ] **初回表示レイテンシ対応一式の実機確認**。build 39 の確認後に、キャッシュ先出し（案B）とそれに伴う修正を実装したため、確認範囲が大きく広がっている。**確認項目は [初回表示レイテンシと版チェックのサーバー往復](decisions/initial-render-latency-version-check.md) の「R9: 受け入れ基準」表（15シナリオ）を正とする。** 集計タブの空メッセージ（issue #9 の未解消分）もその表に含まれる
- [ ] 上記に加えて [開発環境セットアップ](development-setup.md#dev-clientでの動作確認) の定型7項目を確認する
