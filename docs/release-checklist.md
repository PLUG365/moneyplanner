...
- [x] 履歴検索の適用中に件数・合計金額が正しく表示されることを実機で確認する（issue #8、2026-07-25確認済み）
- [x] 履歴タブの初期表示で空メッセージがちらつかないことを実機で確認する（issue #9、2026-07-25確認済み）
- [ ] **集計タブ**の初期表示で空メッセージがちらつかないことを実機で確認する（issue #9。1.0.2の初回ビルドでは未解消だったため `useCachedTransactions` に `hasSettled` を追加。再ビルドして再確認する）
- [ ] 初回表示のタイムラグは1.0.2では対応しない（`docs/decisions/initial-render-latency-version-check.md` で別タスク化。リリース判断には影響しない）
