...
- [ ] **バージョンレコード名を入力したら、ビルドのバージョン表記と一致しているか目視で確認する。** レコード名は自由入力で、バイナリのバージョンと違っていても登録できてしまう（1.0.1 で実際に起きた）
...
- [ ] 必要ならApp Review Informationのメモ欄に、世帯参加中ユーザーの削除導線（設定 → アカウント）を明記する
- [ ] 世帯未参加ユーザー向けの削除導線（`app/household.tsx` の「アカウントを削除」）が従来どおり動作することを確認する
...
- [ ] production ビルドを作成し、TestFlight で最終確認する
- [ ] App Store へ提出する（バージョンレコード作成時に `1.0.2` が受け付けられるか確認。上の項目を参照）
