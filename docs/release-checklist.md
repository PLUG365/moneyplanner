...
- [ ] App Store Connectでバージョンレコード `1.0.2` を作成できることを確認する（現在の公開バージョンのレコード名が `1.01` のため、Appleが `1.01` を `1.1` と解釈して `1.0.2` を「前回より低い」と判定する可能性がある。弾かれた場合は `1.1.1` など `1.01` より大きい番号に変更する。ビルドの `CFBundleShortVersionString` は `1.0.1 (37)` だったため、番号の整合は本来1.0.2で正しい）
