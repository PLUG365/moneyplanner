...
- [x] App Store審査却下（Guideline 5.1.1(v)、2026-07-15受領）への対応と再提出
  ...
  - [x] TestFlight/dev-client実機で、世帯未参加時・世帯参加時それぞれのアカウント削除フローを確認する
...
- [x] App Store Connectでバージョンレコード `1.0.2` を作成できることを確認する（2026-08-03、**問題なく作成・提出できた**）。前バージョンのレコード名が `1.01` だったため「前回より低い」と判定される懸念があったが、実際には弾かれなかった。なお `1.01` はレコード作成時の入力ミス（`1.0.1` と入れるつもりだった）で、ビルド自体は `1.0.1 (37)` だった
- [x] **バージョンレコード名を入力したら、ビルドのバージョン表記と一致しているか目視で確認する。** レコード名は自由入力で、バイナリのバージョンと違っていても登録できてしまう（1.0.1 で実際に起きた）
- [x] 実機で「設定 → アカウントセクションを開く → アカウントを削除」が明確に操作できることを確認する（2026-07-25、1.0.2ビルドで確認済み。`docs/decisions/account-section-default-collapsed.md` の決定でセクションを既定閉じへ戻したため、Guideline 5.1.1(v)の再却下リスクを確認した）
- [x] 必要ならApp Review Informationのメモ欄に、世帯参加中ユーザーの削除導線（設定 → アカウント）を明記する
- [x] 世帯未参加ユーザー向けの削除導線（`app/household.tsx` の「アカウントを削除」）が従来どおり動作することを確認する
- [x] 履歴検索の適用中に件数・合計金額が正しく表示されることを実機で確認する（issue #8、2026-07-25にbuild 39で確認済み）
- [x] 履歴タブの初期表示で空メッセージがちらつかないことを実機で確認する（issue #9、2026-07-25にbuild 39で確認済み）
- [x] **初回表示レイテンシ対応一式の実機確認**（2026-07-26、dev-clientビルドで完了）。確認項目は [初回表示レイテンシと版チェックのサーバー往復](decisions/initial-render-latency-version-check.md) の「R9: 受け入れ基準」表。確認の過程で O-6 / O-7 / O-8 / O-9 と店舗候補の網羅性の問題が判明し、いずれも修正のうえ再確認済み。集計タブの空メッセージ（issue #9 の未解消分）も解消を確認
- [x] 上記に加えて [開発環境セットアップ](development-setup.md#dev-clientでの動作確認) の定型7項目を確認する（2026-07-26）
- [x] production ビルドを作成し、TestFlight で最終確認する
- [x] App Store へ提出する（バージョンレコード作成時に `1.0.2` が受け付けられるか確認。上の項目を参照）
