...
- 収支管理: 手動入力（メイン）
- 履歴: 一覧リスト / カレンダービュー（日付タップで詳細）、長押し複数選択と一括コピー。一覧は既定で当日までを表示（設定で切替）。検索は種別・口座・カテゴリ・内訳・お店・メモ・日付範囲で絞り込み
- 集計: 月次・年次・カテゴリ別・口座別の集計表
...
| 記録（初期画面） | 収支入力フォーム                  |
| 履歴             | リスト表示 / カレンダービュー     |
| 集計             | 月次・年次・カテゴリ別・口座別 + 予算進捗 |
| 設定             | カテゴリ管理・CSV入出力・世帯管理・履歴の表示設定 |

> ライフプラン（旧計画タブ）は廃止しました。詳細は [docs/decisions/plan-feature-retirement.md](docs/decisions/plan-feature-retirement.md) を参照。

### 履歴・集計の表示仕様

- 履歴一覧は既定で当日までの記録を表示する。先日付で登録した予定（給料日・引き落とし日など）が初期表示の先頭を占め、直近の記録が見えなくなるのを避けるため（Issue #13）。判定は `lib/historyFutureVisibility.ts`
  - 設定タブの「未来の記録を表示」をオンにすると上限を外す。端末ローカル保存（`lib/historyDisplayPreference.ts`）で、世帯では共有しない
  - 検索で終了日を明示指定した場合は、その指定を優先して上限を足さない
  - カレンダービューは月を選ぶ操作自体が期間指定のため、この上限を適用しない
- 履歴の一括コピーは、コピー元のカテゴリが現存しなくてもコピーを成立させる。カテゴリID・スナップショット名をそのまま引き継ぎ、完了通知で「カテゴリが存在しないN件」を知らせる（Issue #12）。取引はカテゴリ削除後もスナップショットで表示できる方針のため、コピーだけを失敗させない
  - 「一部コピーされませんでした」一覧は、保存自体に失敗した記録のためのものとして残す
- 履歴の検索条件に口座を加える（Issue #11）。口座名の完全一致で絞り込み、既存の検索合計表示（件数・収入・支出）がそのまま口座ごとの集計になる。候補は取引のスナップショット名から作るため、口座マスタから消えた口座も候補に残る
  - 口座は種別・カテゴリと独立した軸なので、カテゴリの絞り込み条件の外に置く
- 集計タブの月次表示にも口座別集計（収入/支出/収支）を出す（Issue #11）。口座名は取引のスナップショットで束ね、口座を消しても過去の集計名が残る。判定は `lib/accountSummaryAggregation.ts`
...
- [x] 旧レコードコピー時のスナップショット名によるカテゴリ/内訳再解決（カテゴリが現存しない場合もスナップショットのままコピーする / Issue #12）
- [x] 集計タブ（月次・年次・カテゴリ別・口座別）
- [x] 口座管理（収支の出し入れ先）
- [x] 設定タブ（カテゴリ管理・CSV出力）
- [x] iPhoneでの動作確認
...
- [x] 実装済み: 履歴検索の適用中は、リスト上部に件数と合計金額（収入/支出、混在時は差引）を表示する。検索条件ありのときは全件取得されるため合計が正確になる。条件なしの履歴はページング読みで手元に一部しかないため表示しない（issue #8、実機確認予定）
