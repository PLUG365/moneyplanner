...
- 収支管理: 手動入力（メイン）
- 履歴: 一覧リスト / カレンダービュー（日付タップで詳細）、長押し複数選択と一括コピー。一覧は既定で当日までを表示（設定で切替）
...
- 集計タブの月次表示に口座別集計（収入/支出/収支）を出す（Issue #11）。口座名は取引のスナップショットで束ね、口座を消しても過去の集計名が残る。判定は `lib/accountSummaryAggregation.ts`
